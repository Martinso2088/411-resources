...
# Variables
IMAGE_NAME="myapp"  # Replace with your image name
CONTAINER_TAG="latest"  # Replace with the tag you want (e.g., latest, v1.0)
HOST_PORT=5000
CONTAINER_PORT=5000
DB_VOLUME_PATH="/path/to/database"  # Adjust this to the desired host path for the database persistence
BUILD=true  # Set this to true if you want to build the image, false to skip building

# Check if we need to build the Docker image
if [ "$BUILD" = true ]; then
  echo "Building Docker image..."
  docker build -t ${IMAGE_NAME}:${CONTAINER_TAG} .  # Build the image using the Dockerfile in the current directory
else
...
if [ ! -d "${DB_VOLUME_PATH}" ]; then
  ...
  mkdir -p ${DB_VOLUME_PATH}  # Create the directory if it does not exist
fi
...
docker run -d --name ${IMAGE_NAME}_container -p ${HOST_PORT}:${CONTAINER_PORT} -v ${DB_VOLUME_PATH}:/var/lib/postgresql/data ${IMAGE_NAME}:${CONTAINER_TAG}
